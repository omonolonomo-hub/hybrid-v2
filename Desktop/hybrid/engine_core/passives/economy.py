...
def _passive_exoplanet(card: "Card", trigger: str, owner: "Player", opponent: "Player", ctx: dict) -> int:
    ...
    if trigger == "income" and owner is not None:
        # H3-2: Support market_window in ctx (independent TurnManager)
        # Fallback to owner.market for backward compatibility
        market_window = ctx.get("market_window") or getattr(owner, "market", [])
        if any(c and c.rarity in ("4", "5") for c in market_window):
            owner.gold += 1
            owner.stats["gold_earned"] = owner.stats.get("gold_earned", 0) + 1
    return 0
