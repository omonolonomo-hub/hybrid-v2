...
class StrategyLogger:
    ...
    def end_game(self, game: "Game", winner: "Player"):
        ...
        for player in game.players:
            try:
                ...
                logger.warning(
                    "Error aggregating passive data for player=%s game_id=%s: %s",
                    getattr(player, "pid", "unknown"),
                    self._game_id,
                    e,
                )
                continue
    ...
    def _write_passive_efficiency_kpi(self):
        ...
        try:
            ...
            logger.warning("Failed to write passive_efficiency_kpi.jsonl: %s", e)
        except Exception as e:
            logger.exception("Unexpected error in passive KPI generation: %s", e)
