...
from engine_core.constants import MARKET_REFRESH_COST
...
class Market:
    def __init__(self, pool: List[Card], rng=None):
        ...
        self._player_windows: Dict[int, List[Optional[Card]]] = {}  # pid -> open window
        self.rng = rng if rng is not None else random.Random()
        self._current_turn: int = 1  # updated externally if needed

    def get_window(self, pid: int) -> List[Optional[Card]]:
        """Public API to get the current market window for a player."""
        return self._player_windows.get(pid, [])

    def get_rarity_weight(self, rarity: str, turn: int) -> float:
        """Public proxy for rarity weight calculation."""
        return _rarity_weight(rarity, turn)

    def clear_slot(self, pid: int, slot_idx: int):
        """Public API to clear a slot after purchase."""
        window = self.get_window(pid)
        if 0 <= slot_idx < len(window):
            window[slot_idx] = None
