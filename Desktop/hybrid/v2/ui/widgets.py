...
class FloatingText:
    ...
    def render(self, surface: pygame.Surface) -> None:
        ...
        # ── Katman 1: Pill halo (Cached) ─────────────────────────────
        pad    = 5
        halo_w = tw + pad * 2
        halo_h = th + pad * 2
        halo_a = int(89 * a / 255)      # 255 × 0.35 ≈ 89
        if halo_a > 0:
            halo_key = (halo_w, halo_h)
            if halo_key not in FloatingText._halo_cache:
                base = pygame.Surface((halo_w, halo_h), pygame.SRCALPHA)
                pygame.draw.rect(
                    base,
                    (0, 0, 0, 255),
                ...
                FloatingText._halo_cache[halo_key] = base
            
            cached_halo = FloatingText._halo_cache[halo_key].copy()
            cached_halo.set_alpha(halo_a)
            surface.blit(cached_halo, (x - pad, y - pad))
