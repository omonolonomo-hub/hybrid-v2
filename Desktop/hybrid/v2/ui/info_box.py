...
class InfoBox:
    def __init__(self, rect: pygame.Rect) -> None:
        ...
        # Önbelleğe alınmış yüzey (Her frame'de tahsisatı önlemek için)
        self._box_surf = pygame.Surface((self.rect.w, self.rect.h), pygame.SRCALPHA)
    ...
    def render(self, surface: pygame.Surface) -> None:
        ...
        # Lerp (150ms hedef)
        lerp_speed = 10.0
        self._alpha    = min(1.0, max(0.0, self._alpha + (1.0 - self._alpha) * lerp_speed * dt))
        ...
        # Önbelleğe alınmış yüzeyi temizle ve kullan
        self._box_surf.fill((0, 0, 0, 0))
    ...
    def _render_card(self, surface: pygame.Surface, card: CardData, rect: pygame.Rect) -> None:
        ...
        # ── 1. İsim (Yeni font uygulandı)
        name_r = pygame.Rect(inner.x, y, inner.w, line_h)
        font_cache.render_text(surface, card.name, font_cache.card_name(16), (*cat_color, a_int), name_r)
        ...
        font_cache.render_text(surface, f"[{card.category}]", font_cache.other_texts(11), tag_cat_color, tag_r, align="left")
        ...
        if card.synergy_group:
            ...
            font_cache.render_text(surface, f"<{card.synergy_group}>", font_cache.other_texts(11), grp_color, tag_r, align="right")
        else:
            font_cache.render_text(surface, f"<INDEPENDENT>", font_cache.other_texts(11), (160, 160, 170), tag_r, align="right")
        ...
        # [GÜNCELLEME] Diğer metinler için yeni font
        p_font = font_cache.other_texts(11)
        ...
        # Diğer metinler için yeni font
        font_cache.render_text(surface, label, font_cache.other_texts(12), passive_color, passive_lbl_r)
        ...
        for i, (stat_name, val) in enumerate(stats[:6]):
            ...
            # Stat değeri için yeni font
            font_cache.render_text(surface, str(int(val)), font_cache.other_texts(12), val_color, val_r, align="left")
