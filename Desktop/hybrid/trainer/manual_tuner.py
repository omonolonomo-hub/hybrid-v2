...
from v2.core.exceptions import EngineException
...
def setup_baseline(force: bool = False,
    ...
    * Otherwise writes `params` (or BASELINE_PARAMS as fallback) →
    ...
    if not _run_sim():
        raise EngineException("Baseline simulation failed — cannot proceed with tuning")

    sim_summary = safe_read_json(SIM_OUTPUT)
    if sim_summary is None:
        raise EngineException(f"Baseline output missing: {SIM_OUTPUT}")
