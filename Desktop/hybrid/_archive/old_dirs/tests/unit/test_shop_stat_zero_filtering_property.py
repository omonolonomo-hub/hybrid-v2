"""
Property-based tests for shop stat zero filtering.

This test validates that stats with zero values are NOT rendered
in the shop card stat grid, ensuring visual clarity by hiding inactive stats.

Feature: board-shop-ui-cleanup-v3
Task: 5.2 Write property test for shop stat zero filtering
...
# Strategy for generating card data with some zero-value stats
def card_with_zero_stats_strategy():
    """Generate cards with at least one zero-value stat."""
...
class TestShopStatZeroFiltering:
    """Property-based tests for shop stat zero filtering."""
    ...
    @given(card=card_with_zero_stats_strategy())
    def test_zero_stats_not_rendered_in_shop_grid(self, card):
        """
        **Validates: Requirements 3.2**
        
        Property 5: Shop Stat Zero Filtering
        
        For any card in the shop, stats with value == 0 SHALL NOT appear 
        in the rendered stat grid.
        
        This test generates cards with some zero-value stats, renders them
        as shop cards, and verifies that no zero-value stats are visible
        in the stat grid.
        """
        # Get all stats from the card
        edge_source = getattr(card, "edges", None) or list(getattr(card, "stats", {}).items())
        zero_stats = [(name, val) for name, val in edge_source 
                      if val == 0 and not str(name).startswith("_")]
        
        # Skip cards with no zero stats (nothing to test)
        assume(len(zero_stats) > 0)
        ...
        # Verify no zero-value stats are rendered
        incorrectly_rendered = []
        for stat_name, value in zero_stats:
            if stat_name in rendered_stat_names:
        ...
        assert len(incorrectly_rendered) == 0, (
            f"Zero-value stats were incorrectly rendered in shop card for {card.name}.\n"
            f"Card rarity: {card.rarity}\n"
            f"All stats: {dict(edge_source)}\n"
            f"Zero stats: {zero_stats}\n"
    ...
    def test_specific_card_with_all_zeros(self):
        """
        Deterministic test: verify a card with all zero stats.
        ...
        assert len(rendered_stat_names) == 0, (
            f"Expected no stats to be rendered for all-zero card, "
            f"but found: {rendered_stat_names}"
        )
    
    def test_mixed_card_zero_stats_not_rendered(self):
        """
        Deterministic test: verify a card with mixed zero/non-zero stats.
        Focus on verifying zero stats are not rendered.
        """
        ...
        zero_stat_names = ["Durability", "Speed", "Secret"]
        
        for stat_name in zero_stat_names:
            assert stat_name not in rendered_stat_names, (
                f"Expected stat {stat_name} (value=0) to NOT be rendered, "
                f"but it was detected. Rendered stats: {rendered_stat_names}"
            )
    ...
    def test_single_zero_stat(self):
        """
        Deterministic test: verify a card with only one zero stat.
        """
        ...
        # Only Secret should NOT be rendered
        assert "Secret" not in rendered_stat_names, (
            f"Expected stat Secret (value=0) to NOT be rendered, "
            f"but it was detected. Rendered stats: {rendered_stat_names}"
        )
