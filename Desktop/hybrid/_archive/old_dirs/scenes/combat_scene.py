...
from ui.hand_panel import HandPanel  # T3.2d: Hand panel integration
...
class PlacementController:
    ...
    def is_valid_placement(self, hex_coord: Tuple[int, int], current_player_id: int) -> bool:
        ...
        3. Hex is not locked (not in locked_coords_per_player)
        4. Hex is within player's board area
        ...
        - T3.5: Check if hex is not locked
        """
        q, r = hex_coord
    ...
    def handle_left_click(self, hex_coord: Tuple[int, int], current_player_id: int, 
        ...
        - T3.5: Add coord to locked_coords_per_player after placement
        """
        # Validate placement
...
class CombatScene(Scene):
    ...
    BOTTOM_HUB_HEIGHT = 175  # Updated from 150 to 175 for hand panel (T3.2d)
    TOP_MARGIN = 50
    PLACE_PER_TURN = 1  # Maximum cards that can be placed per turn (T3.4)
    ...
    def on_enter(self) -> None:
        ...
        # Initialize current_player_id from view_player_index
        self.current_player_id = self.core_game_state.view_player_index
        ...
        print(f"✓ HandPanel initialized with AssetLoader")
    ...
    def on_exit(self) -> None:
        ...
        # T3.4c: Reset placement counter for next turn
        if self.ui_state is not None:
            self.ui_state.placed_this_turn = 0
            print(f"✓ Placement counter reset to 0")
    ...
    def handle_input(self, input_state: InputState) -> None:
        ...
        if player_switch_request != -1:
            ...
            if 0 <= player_switch_request < num_players:
                ...
                # Rebuild hex cards for new player's board
                self._build_hex_cards()
                ...
                print(f"✓ Board rebuilt: {len(self.hex_cards)} cards on new player's board")
            else:
    ...
    def _handle_input_normal(self, input_state: InputState) -> None:
        ...
        - Left-click on hand panel to select card (T3.3b)
        - R key or right-click to rotate selected card (T3.3c)
        ...
        - T3.3b: Left-click on hand panel to select/deselect cards
        - T3.3c: R key or right-click to rotate selected card
        """
        # T3.3c: Handle R key or right-click to rotate selected card
        if self.ui_state.selected_hand_idx is not None:
            ...
            if rotate_requested:
                # Increment pending_rotation (mod 6 for 0-5 range)
                self.ui_state.pending_rotation = (self.ui_state.pending_rotation + 1) % 6
                rotation_degrees = self.ui_state.pending_rotation * 60
                print(f"✓ Card rotation: {rotation_degrees}° (step {self.ui_state.pending_rotation}/6)")
        ...
        if input_state.mouse_clicked:
            ...
            # T3.3b: Handle left-click on hand panel to select/deselect cards
            mouse_x, mouse_y = mouse_pos
            card_idx = self.hand_panel.get_card_at_position(mouse_x, mouse_y)
            
            if card_idx is not None:
                # Card was clicked in hand panel
                if self.ui_state.selected_hand_idx == card_idx:
                    # Same card clicked - deselect
                    self.ui_state.selected_hand_idx = None
                    self.ui_state.pending_rotation = 0
                    print(f"✓ Card deselected")
                else:
                    # Different card clicked - select and start placement
                    current_player = self.core_game_state.current_player
                    if card_idx < len(current_player.hand):
                        card = current_player.hand[card_idx]
                        self.ui_state.selected_hand_idx = card_idx
                        # Copy card's current rotation to pending_rotation
                        self.ui_state.pending_rotation = card.rotation
                        
                        # Start placement mode
                        self._start_placement(card)
                        
                        print(f"✓ Card selected: {card.name} (index {card_idx}, rotation {card.rotation})")
                        print(f"✓ Placement mode started - click on board to place")
                return  # Don't process other click handling
        ...
        # NOTE: This is now handled above in T3.3b section
    ...
    def _handle_input_placing(self, input_state: InputState) -> None:
        ...
        # Left-click to place card (Task 28.2, T3.4b)
        if input_state.mouse_clicked:
            # T3.4b: Check placement limit before attempting placement
            if self.ui_state.placed_this_turn >= self.PLACE_PER_TURN:
                remaining = self.PLACE_PER_TURN - self.ui_state.placed_this_turn
                print(f"✗ Placement limit reached: {self.ui_state.placed_this_turn}/{self.PLACE_PER_TURN} cards placed this turn")
                # Show status message to user
                if hasattr(self.ui_state, 'set_message'):
                    self.ui_state.set_message(f"Placement limit: {remaining} remaining", 2000)
                return
            ...
            if hex_coord in self.hex_grid:
                ...
                if success:
                    # T3.4b: Increment placement counter
                    self.ui_state.placed_this_turn += 1
                    remaining = self.PLACE_PER_TURN - self.ui_state.placed_this_turn
                    ...
                    print(f"✓ Placements: {self.ui_state.placed_this_turn}/{self.PLACE_PER_TURN} ({remaining} remaining)")
                else:
    ...
    def _get_card_at_hand_position(self, mouse_x: int, mouse_y: int):
        """Get card from hand at mouse position.
        
        Checks if mouse position is within the hand area and returns
        the card at that position if found.
        ...
        Note:
            This is a placeholder implementation. The actual hand rendering
            and card selection logic will be implemented in a future task.
            For now, this returns None to allow the placement system to be tested
            with programmatic card selection.
        """
        # TODO: Implement hand area detection and card selection
        # Hand area is typically at the bottom of the screen
        # For now, return None (no card selection from hand yet)
        return None
    ...
    def _draw_locked_indicators(self, screen: pygame.Surface) -> None:
        ...
        This is Layer 3.5 (Z=3.5) - rendered after cards, before hover highlights.
        
        Args:
            screen: Pygame surface to draw on
        
        Requirements:
        - T3.5: Display locked indicator with C_LOCKED color on locked hexes
        """
        # Get current player's locked coordinates
        current_player_id = getattr(self, 'current_player_id', 0)
        locked_coords = self.core_game_state.locked_coords_per_player.get(current_player_id, set())
        
        # C_LOCKED color (orange) from run_game.py and board_renderer_v3.py
        C_LOCKED = (255, 180, 40)
        
        # Draw locked border for each locked hex
        for hex_coord in locked_coords:
            # Only draw if hex is in the grid
            if hex_coord in self.hex_grid:
    ...
    def _draw_left_panel(self, screen: pygame.Surface) -> None:
        """Draw left panel with synergy badges and passive buff log.
        
        Displays compact synergy badges (icon + count) and scrollable passive buff log.
        Panel has semi-transparent dark background with cyan border glow.
        Modern autochess layout: left sidebar for synergy/buff info.
        ...
        - T3.1: Synergy badges and passive buff panel on left side
        """
        from ui.hud_renderer import draw_synergy_hud, draw_passive_buff_panel
        ...
        # Initialize fonts for HUDRenderer
        ...
        fonts = {
            ...
            "sm": _font("consolas", 13, bold=False),
        }
        
        # Get current player
        current_player = self.core_game_state.current_player
        
        # T3.1a: Draw synergy badges (compact style with icons)
        # Position at x=20-30px from left edge, y=20px from top
        synergy_x = 30
        synergy_y = panel_y + 20
        synergy_height = draw_synergy_hud(
            screen,
            current_player,
            fonts,
            hovered_group=None
        )
        # Note: draw_synergy_hud positions itself at bottom center
        # synergy_height is returned but position is fixed by the function
        
        # T3.1b: Draw passive buff panel below synergy badges
        # Position below synergy panel with 20px gap
        passive_x = 30
        passive_y = synergy_y + synergy_height + 20
        max_passive_height = panel_height - (synergy_height + 60)  # Reserve space for synergy + margins
        
        passive_height = draw_passive_buff_panel(
            screen,
            current_player,
            fonts["sm"],
            passive_x,
    ...
    def _draw_bottom_hub(self, screen: pygame.Surface) -> None:
        """Draw bottom hub with strategic information.
        
        Displays economy stats (gold, income), combat stats (combo, power),
        and synergy information. Panel has semi-transparent dark background
        with white top border glow.
        
        Args:
            screen: Pygame surface to draw on
        
        Requirements:
        - 2.3: Bottom hub 150px height for strategic information
        - 10.1: Semi-transparent dark background with white top border
        """
        # Panel dimensions
        panel_x = 0
        panel_y = self.SCREEN_HEIGHT - self.BOTTOM_HUB_HEIGHT
        panel_width = self.SCREEN_WIDTH
        panel_height = self.BOTTOM_HUB_HEIGHT
        
        # Create semi-transparent surface
        panel_surface = pygame.Surface((panel_width, panel_height), pygame.SRCALPHA)
        panel_surface.fill((20, 20, 40, 220))  # Dark with higher alpha
        
        # Draw panel background
        screen.blit(panel_surface, (panel_x, panel_y))
        
        # Draw white top border glow (2px)
        border_color = (255, 255, 255)
        pygame.draw.line(screen, border_color, (panel_x, panel_y), (panel_x + panel_width, panel_y), 2)
        ...
        # Component 1: Gold Display (x_offset: 50)
        x_offset = 50
        y_center = panel_y + panel_height // 2
        
        # Draw gold icon placeholder (32x32 circle)
        icon_x = panel_x + x_offset
        icon_y = y_center - 16
        pygame.draw.circle(screen, (255, 215, 0), (icon_x + 16, icon_y + 16), 16)
        pygame.draw.circle(screen, (200, 170, 0), (icon_x + 16, icon_y + 16), 16, 2)
        
        # Gold text
        gold_text = f"Gold: {current_player.gold}"
        gold_surface = large_font.render(gold_text, True, (255, 215, 0))
        screen.blit(gold_surface, (icon_x + 40, y_center - 12))
        
        # Component 2: Income Display (x_offset: 250)
        x_offset = 250
        
        # Calculate income (simplified)
        base_income = 5  # BASE_INCOME constant
        streak_bonus = current_player.win_streak // 3
        total_income = base_income + streak_bonus
        
        # Draw income icon placeholder (24x24 arrow up)
        icon_x = panel_x + x_offset
        icon_y = y_center - 12
        arrow_points = [(icon_x + 12, icon_y), (icon_x + 24, icon_y + 12), (icon_x + 18, icon_y + 12),
                       (icon_x + 18, icon_y + 24), (icon_x + 6, icon_y + 24), (icon_x + 6, icon_y + 12),
                       (icon_x, icon_y + 12)]
        pygame.draw.polygon(screen, (0, 255, 0), arrow_points)
        
        # Income text
        income_text = f"Income: +{total_income}"
        income_surface = medium_font.render(income_text, True, (0, 255, 0))
        screen.blit(income_surface, (icon_x + 30, y_center - 10))
        
        # Component 3: Combo Score (x_offset: 450)
        x_offset = 450
        
        # Get combo score from player stats
        combo_score = current_player.stats.get("combo_triggers", 0)
        
        # Draw star icon placeholder (28x28)
        icon_x = panel_x + x_offset
        icon_y = y_center - 14
        pygame.draw.circle(screen, (255, 165, 0), (icon_x + 14, icon_y + 14), 14)
        pygame.draw.circle(screen, (200, 130, 0), (icon_x + 14, icon_y + 14), 14, 2)
        
        # Combo text
        combo_text = f"Combo: {combo_score}x"
        combo_surface = medium_font.render(combo_text, True, (255, 165, 0))
        screen.blit(combo_surface, (icon_x + 35, y_center - 10))
        
        # Component 4: Combat Power (x_offset: 650)
        x_offset = 650
        
        # Calculate combat power (sum of board card stats)
        combat_power = 0
        for card in current_player.board.grid.values():
            if card is not None:
                # Sum all edge stats (edges is a list of tuples: [(name, value), ...])
                combat_power += sum(value for _, value in card.edges)
        
        # Draw sword icon placeholder (32x32)
        icon_x = panel_x + x_offset
        icon_y = y_center - 16
        sword_points = [(icon_x + 16, icon_y), (icon_x + 20, icon_y + 10), (icon_x + 28, icon_y + 10),
                       (icon_x + 28, icon_y + 14), (icon_x + 20, icon_y + 14), (icon_x + 18, icon_y + 32),
                       (icon_x + 14, icon_y + 32), (icon_x + 12, icon_y + 14), (icon_x + 4, icon_y + 14),
                       (icon_x + 4, icon_y + 10), (icon_x + 12, icon_y + 10)]
        pygame.draw.polygon(screen, (255, 0, 0), sword_points)
        
        # Power text
        power_text = f"Power: {combat_power}"
        power_surface = large_font.render(power_text, True, (255, 0, 0))
        screen.blit(power_surface, (icon_x + 40, y_center - 12))
        
        # Component 5: Active Synergies (x_offset: 900)
        x_offset = 900
        
        # Label
        label = medium_font.render("Synergies:", True, (200, 200, 200))
        screen.blit(label, (panel_x + x_offset, y_center - 10))
        
        # Placeholder synergy icons (32x32 each, horizontal row)
        icon_size = 32
        icon_spacing = 5
        for i in range(6):
            icon_x = panel_x + x_offset + 120 + i * (icon_size + icon_spacing)
            icon_y = y_center - 16
            
            # Draw colored placeholder square
            color = [(255, 100, 100), (100, 255, 100), (100, 100, 255), 
                     (255, 255, 100), (255, 100, 255), (100, 255, 255)][i]
            pygame.draw.rect(screen, color, (icon_x, icon_y, icon_size, icon_size))
            pygame.draw.rect(screen, (255, 255, 255), (icon_x, icon_y, icon_size, icon_size), 1)
        
        # Component 6: Potential Synergies (x_offset: 1400)
        x_offset = 1400
        
        # Label (grayed out)
        label = small_font.render("Potential:", True, (120, 120, 120))
        screen.blit(label, (panel_x + x_offset, y_center - 8))
        
        # Placeholder potential synergy icons (24x24 each, semi-transparent)
        icon_size = 24
        icon_spacing = 5
        for i in range(4):
            icon_x = panel_x + x_offset + 100 + i * (icon_size + icon_spacing)
            icon_y = y_center - 12
            
            # Draw semi-transparent colored placeholder square
            icon_surface = pygame.Surface((icon_size, icon_size), pygame.SRCALPHA)
            color = [(255, 100, 100, 128), (100, 255, 100, 128), 
                     (100, 100, 255, 128), (255, 255, 100, 128)][i]
            icon_surface.fill(color)
            screen.blit(icon_surface, (icon_x, icon_y))
            pygame.draw.rect(screen, (150, 150, 150), (icon_x, icon_y, icon_size, icon_size), 1)
    ...
    def _render_placement_preview(self, screen: pygame.Surface) -> None:
        ...
        Shows preview when:
        1. In placement mode (is_placing=True) with PlacementController
        2. OR when card selected from hand (selected_hand_idx is not None)
        
        Renders:
        - Invalid hex: red border
        - Valid hex: cyan border + ghost card with rotation and transparency
        - Hover with selected card: C_SELECT highlight (T3.6a)
        ...
        - T3.6a: Hover detection with selected card from hand
        """
        # T3.6a: Check if card selected from hand (new preview mode)
        has_selected_hand_card = (
            self.ui_state.selected_hand_idx is not None and
            self.ui_state.selected_hand_idx < len(self.core_game_state.current_player.hand)
        )
        
        # Check if in placement mode (old preview mode)
        in_placement_mode = self.ui_state.is_placing and hasattr(self, 'placement_controller')
        
        # Exit if neither mode is active
        if not has_selected_hand_card and not in_placement_mode:
            return
        ...
        # T3.6a: Handle preview for selected hand card
        if has_selected_hand_card:
            # Get selected card from hand
            selected_card = self.core_game_state.current_player.hand[self.ui_state.selected_hand_idx]
            ...
            # Simple validation: hex must be empty
            is_valid = hex_coord not in board.grid or board.grid[hex_coord] is None
            
            if not is_valid:
                # Draw red border for invalid hex
                red_color = (255, 50, 50)
                self._draw_hex_border_highlight(screen, hex_coord, red_color, width=3)
                return
            
            # Valid hex: draw C_SELECT (cyan) border (T3.6a)
            C_SELECT = (0, 242, 255)
            self._draw_hex_border_highlight(screen, hex_coord, C_SELECT, width=3)
            
            # Render ghost card at preview hex with pending_rotation
            card_name = selected_card.name
            faces = self.asset_loader.get(card_name)
            front_image = faces.front
            
            # Apply pending_rotation (0-5 range, convert to degrees)
            rotation_degrees = self.ui_state.pending_rotation * 60
            if rotation_degrees != 0:
                rotated_image = pygame.transform.rotate(front_image, -rotation_degrees)
            else:
            ...
            # T3.6c: Render edge stats with rotation applied
            if hasattr(selected_card, 'edges') and selected_card.edges:
                # Calculate edge positions for flat-top hex
                # Edge order: [N, NE, SE, S, SW, NW] (0-5)
                # Apply pending_rotation to edge indices
                edge_angles = [90, 30, 330, 270, 210, 150]  # Degrees for flat-top hex
                edge_distance = self.hex_size * 0.75  # Distance from center to edge
                
                for edge_idx, (stat_name, stat_value) in enumerate(selected_card.edges):
                    # Apply rotation: rotate edge index by pending_rotation steps
                    rotated_edge_idx = (edge_idx + self.ui_state.pending_rotation) % 6
                    angle_deg = edge_angles[rotated_edge_idx]
                    angle_rad = math.radians(angle_deg)
                    
                    # Calculate position
                    stat_x = center_x + edge_distance * math.cos(angle_rad)
                    stat_y = center_y - edge_distance * math.sin(angle_rad)
                    
                    # Render stat with semi-transparent background
                    self._render_stat_text(screen, (stat_x, stat_y), stat_value, stat_name)
            
            return  # Done with hand card preview
        
        # Original placement mode logic (PlacementController)
    ...
    def draw(self, screen: pygame.Surface) -> None:
        ...
        # Layer 5: Draw hand panel (T3.2d: Hand panel integration)
        self.hand_panel.draw(screen)
