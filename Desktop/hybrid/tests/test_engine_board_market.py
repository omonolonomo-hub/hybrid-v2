...
def make_card(name: str, *, group: str = "MIND", rarity: str = "1", value: int = 2) -> Card:
    ...
    make_card._uid += 1
    return Card(name=name, category="Test", rarity=rarity, stats=stats, uid=make_card._uid)
...
def test_rarity_weight_curve_matches_expected_turn_steps():
    market = Market([], rng=random.Random(0))
    assert market.get_rarity_weight("3", 1) == 0.3
    assert market.get_rarity_weight("3", 9) == 1.0
    assert market.get_rarity_weight("4", 1) == 0.0
    assert market.get_rarity_weight("4", 5) == 0.2
    assert market.get_rarity_weight("5", 7) == 0.0
    assert market.get_rarity_weight("5", 18) == 1.0
...
def test_find_combos_counts_each_unique_neighbor_pair_once():
    ...
    right = make_card("Right", group="MIND")
    right.rotate(2)
    board.place((1, 0), right)
...
def test_resolve_single_combat_distributes_prefix_bonus_evenly_across_edges():
    ...
    card_a.set_meta("_prefix_bonus", 6)
