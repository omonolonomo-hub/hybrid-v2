...
def _first_filled_hand_slot(scene: ShopScene) -> int:
    for idx, card_name in enumerate(scene.hand_panel.get_card_names()):
        if card_name:
            return idx
...
def test_main_bootstrap_and_first_frame_smoke():
    ...
    manager.set_scene(ShopScene(gs))
    manager.update(16)
...
def test_shop_scene_update_keeps_core_hud_in_sync_same_frame():
    ...
    # scene = ShopScene(gs) is already done above, removing redundant creation
    # scene = ShopScene() 
    scene.sync_view()
    scene.update(16)
...
@pytest.mark.xfail(reason="H3-7: LobbyPayload HP assertion — mock engine HP 150 vs expected 111", strict=False)
